...
public class IDecideDBHelper extends SQLiteOpenHelper {
	...
	public Cursor getAllScenarios() {
		...
		// return this.getReadableDatabase().query("scenarios", new
		// String[]{"scenario_name"}, null, null, null, null, null);
		return this.getReadableDatabase().rawQuery(
				"select _id, scenario_name from scenarios", null);
	}
	...
	public Cursor getScenarioItems(int scenarioId) {
		...
		return this
				.getReadableDatabase()
				.rawQuery(
						"select _id, item_name from scenario_items where scenario_id = ?",
						new String[] { String.valueOf(scenarioId) });
	}

	public void deleteScenario(int scenarioId) {
		Log.i(TAG, "Begin to delete the scenario: " + scenarioId);
		SQLiteDatabase db = this.getReadableDatabase();
		db.delete("scenario_items", "scenario_id = ?",
				new String[] { String.valueOf(scenarioId) });
		db.delete("scenarios", "_id = ?",
				new String[] { String.valueOf(scenarioId) });
		Log.i(TAG, "Successfully deleted the scenario: " + scenarioId);
	...
	public void insertScenarioItems(int scenarioId, List<String> itemNames) {
		Log.i(TAG, "Begin to insert the scenario items for scenario: "
				+ scenarioId);
		ContentValues values = new ContentValues();
	...
	public void insertScenarioItem(int scenarioId, String itemName) {
		Log.i(TAG, "Begin to insert the scenario items for scenario: "
				+ scenarioId);
		ContentValues values = new ContentValues();
		values.put("scenario_id", scenarioId);
	...
	public void deleteScenarioItem(int itemId) {
		this.getReadableDatabase().delete("scenario_item", "_id = ?", new String[]{String.valueOf(itemId)});
	}
